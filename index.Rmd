...
description: |
  Interactive visualization of different quantum gates applied to a qubit
author:
...
In classical computing, the basic unit is a bit - 0 or 1. In quantum computing, the basic unit is a qubit. Instead of just two values, we have a vector with two components. In this article, we explore what is a single qubit, and what are the quantum computing operations: 
...
1
\end{bmatrix}$$
In general, a qubit can be any superposition of $|0\rangle$ and $|1\rangle$ - that is

$$|\psi\rangle=\alpha|0\rangle +\beta|1\rangle$$

where $\alpha$ and $\beta$ are [complex numbers](https://www.3blue1brown.com/videos-blog/complex-number-fundamentals-lockdown-math-ep-3)
...
If complex numbers are new to you - don’t worry. First, many interesting quantum states, operators, and phenomena can be told without complex numbers. Second, complex numbers are fun on their own.
...
<div id='states-basis'>
  ...
  <div style='font-family: MathJax_Main;'>
    ...
    <p-radio class=\"p-switch p-fill state_r\" name=\"switch1\" color=\"success\" value=\"st_r\" v-model=\"picked\" @change=\"changeRadio\">|R&rangle;</p-radio>

    <p-radio class=\"p-switch p-fill state_l\" name=\"switch1\" color=\"success\" value=\"st_l\" v-model=\"picked\" @change=\"changeRadio\">|L&rangle;</p-radio>
...
<div style='border: none; margin: 0px; padding: none;'>
**2. Pick a gate:**
```{r}
HTML("
<div id='gate_combo'>
...
<div style='border: none; margin-top: -70px; padding: none;'>
**3. Check the result:**
...
Quantum gates can be composed. In a conventional matrix notation, we read from right to left. That is, $X \;H \; Z \; |0\rangle$ means that we start with state $|0\rangle$, then apply $Z$ gate, then $H$, then $X$. It is a historic notation that is going to stay here.

You can interact with various sequences of gates. Simply drag the gate you want to apply to the selected gates area. To delete a gate from that area, drag it to the trash area.

Remember, in this visualization we are using a gate application order from left to right.

You can pick any input state from the gauges. It uses a general parametrization of:

$$|\psi\rangle = \cos\left(\tfrac{\theta}{2}\right) \; |0\rangle + e^{i\phi} \; \sin\left(\tfrac{\theta}{2}\right) \; |1\rangle$$

If you want to avoid complex values, use $\phi=0$ or $\phi=\pi$.

Observe how the state changes according to the applied gates. In BraKetVue, we use colors to denote phase of complex numbers, see [@DomainColoring20], Chapter 3 from [@Migdal14], and [@Qubism20] from QuTip.
...
This post is a part of the quantum computing series by Quantum Flytrap. It uses [Quantum Tensors](https://github.com/Quantum-Game/quantum-tensors) library for computing quantum operations, and [BraKetVue](https://github.com/Quantum-Game/bra-ket-vue) for quantum state and operator visualization. These libraries are open source (MIT), as well as this post (CC-BY). The source to this post is in [here](https://github.com/clausia/flytrap-examples). We believe in an open, contributive culture.

<br>

*[Thanks or so] (this part is still missing)*
...
.pretty.p-switch.p-fill.state_0 input:checked~.state.p-success:before {
    background-color: #ff0055!important;
}
.pretty.p-switch.state_0 input:checked~.state.p-success:before {
    border-color: #ff0055;
}
  
.pretty.p-switch.p-fill.state_1 input:checked~.state.p-success:before {
    background-color: #5c00d3!important;
}
.pretty.p-switch.state_1 input:checked~.state.p-success:before {
    border-color: #5c00d3;
}
  
.pretty.p-switch.p-fill.state_p input:checked~.state.p-success:before {
    background-color: #0071dd!important;
}
.pretty.p-switch.state_p input:checked~.state.p-success:before {
    border-color: #0071dd;
}

.pretty.p-switch.p-fill.state_m input:checked~.state.p-success:before {
    background-color: rgb(97, 219, 239)!important;
}
.pretty.p-switch.state_m input:checked~.state.p-success:before {
    border-color: rgb(97, 219, 239);
}

.pretty.p-switch.p-fill.state_r input:checked~.state.p-success:before {
    background-color: rgb(119, 216, 7)!important;
}
.pretty.p-switch.state_r input:checked~.state.p-success:before {
    border-color: rgb(119, 216, 7);
}

.pretty.p-switch.p-fill.state_l input:checked~.state.p-success:before {
    background-color: rgb(97, 170, 181)!important;
}
.pretty.p-switch.state_l input:checked~.state.p-success:before {
    border-color: rgb(97, 170, 181);
}
